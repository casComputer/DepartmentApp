...
export const assignRollAlphabetically = async ({ course, year }) => {
  try {
    const res = await axios.post("/student/autoAssignRollNoAlphabetically", {
      course,
      year,
    });
    if (res.data?.success) {
      res.data?.students.forEach((s) =>
        useTeacherStore.getState().updateStudentRollNo(s.studentId, s.rollno),
      );
      ToastAndroid.show(
...
export const fetchStudentsByClassTeacher = async ({ teacherId, setStatus }) => {
  try {
    const { data } = await axios.post("/student/fetchStudentsByClassTeacher", {
      teacherId,
    });
