...
export const saveFile = async (localUri, filename, format) => {
  ...
  console.log("saveFile ", format, mimetype);
...
export const downloadFile = async (url, format) => {
...
export const deleteIfExists = async (dirUri, filename) => {
  try {
    ...
    if (foundUri) await FileSystem.StorageAccessFramework.deleteAsync(foundUri);
  } catch (e) {
...
export const openFileInBrowser = async (url) => {
  ...
  if (supported) {
    ...
    ToastAndroid.show(`Don't know how to open this URL: ${url}`, ToastAndroid.LONG);
  }
}