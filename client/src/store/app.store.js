...
export const useAppStore = create(set => ({
    ...
    setGlobalProgressText: globalProgressText => set({
        globalProgressText
    })
}));
...
export const useMultiSelectionList = create((set, get) => ({
    ...
    set(state => {
        ...
        return {
            list: exists
            ? state.list.filter(i => i !== id): [...state.list, id]
        };
    }),
...
export const usePromptStore = create((set) => ({
    ...
    open: ({
        title, message, requireText, onConfirm
    }) =>
    set({
        visible: true,
        title,
...
export const toast = {
    success: (title, message) => useToastStore.getState()._add('success', title, message),
    error: (title, message) => useToastStore.getState()._add('error', title, message),
    warn: (title, message) => useToastStore.getState()._add('warning', title, message),
    info: (title, message) => useToastStore.getState()._add('info', title, message),
    show: ({
        type = 'default', title, message
    }) =>
