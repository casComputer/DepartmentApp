...
export const AssignmentRenderItem = ({
    item
}) => (
    <Pressable
        onPress={() =>
        ...
        <View className="flex-row justify-between items-center">
            ...
            <View>
                <CircularProgress
                    ...
                    progress={
                    item.strength < 1
                    ? 0: (item.submissions?.length / item.strength) * 100
                    }
...
const RejectAcceptBtn = ({
    handleReject, handleAccept
}) => {
    const [rejecting,
        setRejecting] = useState(false);
    const [accepting,
        setAccepting] = useState(false);
...
export const AssignmentShowRenderItem = ({
    ...
    const [isDownloaded,
        setIsDownloaded] = useState(false);
    const [isDownloading,
        setIsDownloading] = useState(false);
    const [rejectionMessage,
        setRejectionMessage] = useState("");
    ...
    return (
        <View className="justify-center rounded-xl bg-card px-4 py-2 gap-1">
            <Text className="font-bold text-2xl text-text ">{item.studentId}</Text>
            ...
            <TouchableOpacity
                ...
                <Text className="px-3 rounded-full text-blue-500 text-xl font-bold text-center pt-2">
                    {!isDownloaded?.exists ? isDownloading ? 'Downloading..': 'Download': "Open File"}
                </Text>
            </TouchableOpacity>
            ...
            <Animated.View style={animatedStyle} className="mt-1">
                <TextInput
                    ...
                    ? item.rejectionMessage || "No reason specified!": rejectionMessage
                    }
