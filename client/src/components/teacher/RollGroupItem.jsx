...
const GroupItem = ({ group, onDelete, onUpdate, getAvailableStudents }) => {
    ...
    return (
        ...
        >
            ...
            <Text className="font-bold text-lg text-center mt-4 text-text">
                Selected Students: {group?.selectedStudents?.length}
            </Text>

            <View className="mt-3 flex-row items-center gap-6 justify-center">
                <TouchableOpacity onPress={() => onDelete(group.id)}>
                    <Text className="font-bold text-red-500 text-md px-4 py-3">
                        Remove Group
                    </Text>
                </TouchableOpacity>

                <TouchableOpacity onPress={openModal}>
                    <Text className="font-bold text-violet-500 text-md px-4 py-3">
                        Add/Remove
                    </Text>
                </TouchableOpacity>
            </View>
